...
第一，把资金交给有耀眼头衔的人类交易员，但这些明星交易员是如此的不可控，就像爆发的超新星一样，耀眼一下就很快没落，目前还没有明星交易员能长期跑过标普500，但是，由于他们个人/团队失误所造成的经济损失却是实实在在由用户来承担。

第二，凭借自己能力交易的人，要么就是全凭情绪的追涨杀跌，要么就是企图从海量的金融新闻数据、老套的rsi\macd中获得圣杯。但结果是，数据和新闻的更新是如此的频繁，人脑很难高频率处理这么多甚至会互相矛盾的数据，还能稳定得出胜率高的结论。

第三，就算是用量化工具，找到一些量化因子，但是也是不可解释的黑箱状态，至于好不容易训练出来的量化因子，什么时候失效也是不可控的状态。

总结下来，散户投资者的痛点可以概括为4点：
...
最近nofi.ai 的AI 交易大赛很火，给了我很多启发——如果让AI 来处理信息“降噪”，摒除“人类情绪”的影响来做决策又会怎样呢？

但Data这个系统并不是nofi 的简单复刻，我在nofi的基础上了还做了更多的改良：增加了qlib+LightGBM 的计算以及风控的系统：把大模型不擅长的计算交付给数理的程序，风险控制部分也单独拎出来，不会完全靠模型的自觉。

总结来说，之前AI 交易产品（如 nofi.ai）的问题在于：
...
优化后Data已经可以实行比nofi 更加稳健、可靠的交易行为，即使在极端行情下，也可以对用户的资产进行有力的保护与提升，并且在最近一个月的试运行中，它已经大幅跑赢比特币的收益率。
